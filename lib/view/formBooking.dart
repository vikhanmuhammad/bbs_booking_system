...
class _FormBookingState extends State<FormBooking> {
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            if (selectedStartTime!.hour + int.parse(packageData.durasiPaket.toString()) <=
              ...
              return ListTile(
                ...
                trailing: Row(
                  ...
                  children: [
                    Text('Rp${formatCurrency(double.parse(packageData.hargaPaket.toString()))}',),
  ...
  List<TimeOfDay> _generateTimeListToday() {
    if (operationalHours == null) return [];

    int openingHour =
        DateTime.now().hour > operationalHours!['opening_time']['hour']
            ? DateTime.now().hour
            : operationalHours!['opening_time']['hour'];
    int closingHour = operationalHours!['closing_time']['hour'];

    List<TimeOfDay> times = [];
    print(DateTime.now().hour.toString() + " " + openingHour.toString());
    if (DateTime.now().hour >= openingHour) {
      if (DateTime.now().minute > 30) {
        for (int hour = openingHour + 2; hour < closingHour - 1; hour++) {
          times.add(TimeOfDay(hour: hour, minute: 0));
        }
      } else {
  ...
  List<int> _getAvailableDurations(TimeOfDay startTime) {
    if (operationalHours == null) return [];

    int closingHour = operationalHours!['closing_time']['hour'];
    int remainingHours = closingHour - startTime.hour;

    List<int> durations = [];
    for (int i = 2; i <= remainingHours; i++) {
      durations.add(i);
    }
    return durations;
  }
      ...
      return AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            SizedBox(height: screenWidth * 0.01),
            Text('Harga: Rp${formatCurrency(double.parse(package['hargaPaket'].toString()))}',),
          ],
        ),
        actions: [
          ElevatedButton(
            ...
            onPressed: () async {
              print("Pesan button clicked"); // Log untuk memeriksa apakah tombol ditekan
              if (selectedIndex == -1 || selectedStartTime == null) {
                ...
                setState(() {
                  _selectedPackage = Paket.fromMap(package); // Update _selectedPackage dengan paket yang dipilih
                });
                ...
                bool isCollision = await bookingService
                    .checkBookingCollision(widget.bookingModel);

                if (isCollision) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                        content: Text(
                            'Waktu booking untuk meja ini bertabrakan dengan booking lain.')),
                  );
                } else {
                  _navigateToDetailBooking(context);
                }
              }
            },
            child: Text(
  ...
  void _navigateToDetailBooking(BuildContext context) {
  String selectedPackageName = _selectedPackage != null ? _selectedPackage!.namaPaket : '';
  String selectedPackagePrice = _selectedPackage != null ? _selectedPackage!.hargaPaket.toString() : '';
  String selectedPackageHours = _selectedPackage != null ? _selectedPackage!.durasiPaket.toString() : '';
  ...
  Widget build(BuildContext context) {
    ...
    List<TimeOfDay> availableTimes =
        selectedIndex == 0 ? _generateTimeListToday() : _generateTimeList();
                  ...
                  child: DropdownButtonFormField<TimeOfDay>(
                    ...
                    onChanged: (TimeOfDay? newTime) {
                      setState(() {
                        ...
                        selectedDuration =
                            null; // Reset duration when start time changes
                      });
                    },
                ...
                Center(
                  child: ToggleButtons(
                    ...
                    onPressed: (index) {
                      setState(() {
                        if(index==0){
                          widget.bookingModel.paket="";
                          _selectedPackage?.namaPaket = "";
                        }
                          ...
                          Padding(
                            ...
                            child: DropdownButtonFormField<int>(
                              ...
                              items: selectedStartTime != null
                                  ? _getAvailableDurations(selectedStartTime!)
                                      .map((int duration) {
